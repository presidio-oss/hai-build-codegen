...
export async function resolveHooksDirectory(
	...
	if (isGlobal) {
		return globalHooksDirOverride || path.join(os.homedir(), "Documents", "HAI", "Hooks")
	}
	...
	if (workspaceName) {
		...
		return path.join(targetWorkspace, ".hairules", "hooks")
	}

	// Single workspace: use getCwd
	const cwd = await getCwd(getDesktopDir())
	return path.join(cwd, ".hairules", "hooks")
}
